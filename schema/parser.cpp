...
void Parser::includeSchema (ParserContext *context, const QString &location)
{
    ...
    qDebug () << "[Parser][includeSchema] Including schema at" << qPrintable (schemaLocation);

    if (provider.get (schemaLocation, fileName)) {
        QFile file (fileName);
        if (!file.open (QIODevice::ReadOnly)) {
            qDebug () << "[Parser][includeSchema] Unable to open file" << qPrintable (file.fileName ());
            return;
        }
...
{
    for ( int i = 0; i < d->_elements.count(); ++i ) {
        if ( d->_elements[ i ].nameSpace() == name.nameSpace() && d->_elements[ i ].name() == name.localName() )
...
{
    for ( int i = 0; i < d->_attributes.count(); ++i ) {
        if ( d->_attributes[ i ].nameSpace() == name.nameSpace() && d->_attributes[ i ].name() == name.localName() )
